...
while ($line = mysqli_fetch_assoc($action)) {
    echo "<tr>" . "<td>" . $line["id"] . "</td>" . "<td>" . $line["email"] . "</td>" . "<td>" . $line["passcode"] .
        "</td>" . "<td>" .
        "<form action='update_table.php' method='get'>
         <button value=$line[id] name='updateid'>UPDATE</button>"
        . "</td>" . "<td>" . "</form>" .
        "<form action='delete_from_table.php' method='get'>
         <button value=$line[id] name='delte'>DELETE</button>" .
        "</form></td>" . "</tr>";
}
